...
const Receive = ({}) => {
  ...
  return (
    <div className="flex flex-col bg-gray-700 rounded-lg p-4 overflow-hidden">
            ...
            <div className={`absolute w-full bg-gray-700 border-2 border-gray-300 rounded my-2 pin-t pin-l ${isNetworkList ? 'block' : 'hidden'} z-10`}>
              <ul className="list-reset">
                {networks.map((network, index) => 
                  (<li key={index} onClick={ () => {setSelectedNetwork(network); setIsNetworkList(false)}}><p className={`p-2 block hover:bg-black-900 cursor-pointer ${selectedNetwork?.name === network?.name && 'bg-black-900'}`}>
                    {network?.name}
                  </p></li>)
                )}
              </ul>
            ...
            <div className="flex justify-between items-center rounded-md bg-black-900 text-base">
              <div className="relative">
                ...
                <div className={`absolute w-full bg-gray-700 border-2 border-gray-300 rounded my-2 pin-t pin-l ${isCoinList ? 'block' : 'hidden'} z-10`}>
                    <ul className="list-reset">
                      {pynths.map(coin => 
                        (<li key={coin.id} onClick={ () => {setSelectedCoin(coin); setIsCoinList(false)}}><p className={`flex space-x-2 p-2 hover:bg-black-900 cursor-pointer ${selectedCoin?.name === coin?.name && 'bg-black-900'}`}>
      ...
      <button className="bg-blue-500 my-6 px-4 py-2 w-full rounded-lg text-center text-white text-2xl lg:w-80 lg:mx-auto" onClick={ () => confirm()} disabled={networkId !== selectedNetwork?.id || !selectedNetwork?.id || totalAmount === 0n}>
        Confirm
      </button>
