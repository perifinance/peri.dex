...
const Submit = ({}) => {
  ...
  const bridgeConfirm = async () => {
    ...
    try {
      if(mySignature === zeroSignature)
        throw Error('Bridge need signature');
      let transaction;
      ...
      transaction = await contracts.signers[contractName[selectedCoin.name]].overchainTransfer( 
        utils.parseEther(payAmount), selectedToNetwork.id, [rsv.r, rsv.s, rsv.v], transactionSettings,
      )
  ...
  return (
    <div className="flex flex-col bg-gray-700 rounded-lg p-4">
      ...
      <button className="bg-blue-500 my-6 px-4 py-2 w-full rounded-lg text-center text-white text-2xl lg:w-80 lg:mx-auto" onClick={ () => bridgeConfirm()} disabled={networkId !== selectedFromNetwork?.id || !selectedToNetwork?.id}>
        Confirm
      </button>
      {!isConnect ? 
