...
function RangeInput({ per, setPerAmount, divide = 4, canInput = false, bgColor="bg-cyan-450", color="text-cyan-450"}:RangeInputProps) {
    ...
    return (
		<div className="flex flex-col my-2 items-center">
			<div className="flex flex-row w-full items-center justify-between">
				...
				{canInput &&
					<div className="flex items-center h-7 border-[0.2px] border-blue-800 rounded-md text-sm ml-1 pr-1 bg-blue-950 ">
						<input
			...
			<div className="flex justify-between mt-1 text-xs w-full text-nowrap">
				{levels.map((level, idx) => (
					<span
						className={`base-1/${divide} text-center cursor-pointer ${
							per === BigInt(level) && color
						}`}
						key={`level_${idx}`}
