...
const Order = ({ openCoinList }) => {
    ...
    const getRate = async () => {
        const [sourceRate, destinationRate] = await Promise.all([
            getExchangeRates(selectedCoins.source.symbol),
            getExchangeRates(selectedCoins.destination.symbol),
        ]);
    ...
    useEffect(() => {
        if (isReady && address && !isNaN(networkId)) {
            getSourceBalance();
        }
        ...
        <div className="max-w-sm">
            <div className="w-full bg-gray-600 rounded-t-lg px-4 py-2">
                <div className="flex py-2 justify-between w-full">
                    ...
                    <div className="text-gray-200">
                        {formatCurrency(exchangeRates, 2)} ($
                        {formatCurrency((exchangeRates * sourceRate) / 10n ** 18n, 2)})
                    </div>
                </div>
            </div>
            <div className="w-full bg-gray-800 rounded-b-lg p-4">
                ...
                <div className="flex items-center space-x-6 rounded-xl">
                    <div
                        className="flex py-3 rounded-lg text-gray-200 font-semibold cursor-pointtext-center m-auto"
                ...
                <div className="flex justify-end text-base tracking-wide text-gray-200 text-xs w-full">
                    <span>${formatCurrency(payAmountToUSD, 2)}</span>
                </div>
                {isValidation || (
                    <div className="flex justify-end text-base tracking-wide text-red-500 text-xs w-full">
                ...
                <div className="flex items-center py-2 space-x-6 rounded-xl">
                    ...
                    <div className="flex bg-gray-200 py-2 w-full rounded-lg">
                        <input
                            ...
                            type="text"
                            value={formatCurrency(receiveAmount, 18)}
                        />
                    </div>
                </div>
                ...
                <div className="py-2 w-full">
                    <div className="flex justify-between">
                        ...
                        />
                        <div className="w-10">
                            <label className="text-gray-200 border border-gray-200 rounded-md text-sm px-1">
                                {convertNumber(per)}%
                            </label>
                        </div>
                    </div>
                    <div className="flex justify-between mt-2 text-xs text-gray-400 w-10/12">
                        <span className="w-8 text-left" onClick={() => setPerAmount(0n)}>
                            0%
                        </span>
                        <span className="w-8 text-center" onClick={() => setPerAmount(25n)}>
                            25%
                        </span>
                        <span className="w-8 text-center" onClick={() => setPerAmount(50n)}>
                            50%
                        </span>
                        <span className="w-8 text-center" onClick={() => setPerAmount(75n)}>
                            75%
                        </span>
                        <span className="w-8 text-right" onClick={() => setPerAmount(100n)}>
                            100%
                        </span>
                    </div>
                </div>
                ...
                <div className="py-2 w-full">
                    <div className="flex justify-between">
                        <input
                            ...
                            onChange={(e) => setLeverage(BigInt(e.target.value))}
                        />
                        <div className="w-10">
                            <label className="text-gray-200 border border-gray-200 rounded-md text-sm px-1">
                                X{convertNumber(leverage)}
                            </label>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6 space-x-6 rounded-xl">
                    <button
                        className="bg-blue-400 p-4 w-full rounded-lg text-center text-inherent"
                        onClick={() => order()}
                    >
                        Confirm
                    </button>
