					...
					<div className="flex flex-col space-y-4 xl:flex-row lg:pl-8 xl:space-x-4 xl:space-y-0">
						...
						<div className="flex space-x-4 w-full justify-end xl:justify-start">
							<div className="relative w-36">
								<div
									className="flex justify-between font-medium space-x-2 cursor-pointer items-center bg-gray-500 rounded-lg h-12 p-2"
									onClick={() => {
										clearOpens();
									...
									<ul className="list-reset">
										{pynths[networkId]?.map((coin) => (
											<li
												key={coin.symbol}
												onClick={() => {
													setSearchOptions({ ...searchOptions, dest: coin.symbol });
													setIsSrcCoinList(false);
												}}
											>
												<span
									...
									<ul className="list-reset">
										{pynths[networkId]?.map((coin) => (
											<li
												key={coin.symbol}
												onClick={() => {
													setSearchOptions({ ...searchOptions, src: coin.symbol });
													setIsDestCoinList(false);
												}}
											>
												<span
							...
							<div className="justify-end hidden sm:flex">
								<div
									className="flex font-medium space-x-2 cursor-pointer items-center bg-gray-500 rounded-lg h-12 w-12 p-2 justify-center"
									onClick={() => {
										clearOpens();
										setSearchOptions({});
									}}
										...
										<tr className="border-b border-gray-500 h-10" key={e.id}>
											...
											<td className="text-center">
												<img
													...
													onClick={() =>
														window.open(
															`https://moonbase.moonscan.io/tx/${e.appendedTxid || e.settledTxid}`,
															"_blank"
														)
													}
												/>
							...
							<ul className="flex rounded justify-center">
								{pages.map((e) => {
									return (
										<li key={e}>
											<a
												className={`block hover:text-white hover:underline text-blue px-3 py-2 cursor-pointer ${
									...
									<div className="font-semibold" key={index}>
										...
										<div className="flex justify-between">
											...
											<div className="flex flex-col leading-none">
												<div className="font-medium text-gray-300 text-right">Holding Quantity</div>
